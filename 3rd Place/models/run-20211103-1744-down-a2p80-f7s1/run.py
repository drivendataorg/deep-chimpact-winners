...
parser.add_float('--aug_percentage',  default=0.8,           help='Probablity of outputting augmented image regardless of total number of augmentations')
parser.add_int('--aug_number',        default=15,            help='Number of train-time augmentations. 0 means no augmentation')
...
# v2
def aug_10(image, label): # gray
    image = tf.image.rgb_to_grayscale(image)
    image = tf.image.grayscale_to_rgb(image)
    return image, label
def aug_11(image, label): # gray + high_bright_0.4
    image = tf.image.rgb_to_grayscale(image)
    image = tf.image.grayscale_to_rgb(image)
    image = tf.image.adjust_brightness(image, 0.4)
    return image, label
def aug_12(image, label): # gray + high_bright_0.8
    image = tf.image.rgb_to_grayscale(image)
    image = tf.image.grayscale_to_rgb(image)
    image = tf.image.adjust_brightness(image, 0.8)
    return image, label
def aug_13(image, label): # gray + low_bright_0.4
    image = tf.image.rgb_to_grayscale(image)
    image = tf.image.grayscale_to_rgb(image)
    image = tf.image.adjust_brightness(image, -0.4)
    return image, label
def aug_14(image, label): # hue_0.4
    image = tf.image.adjust_hue(image, 0.4)
    return image, label
def aug_15(image, label): # hue_0.8
    image = tf.image.adjust_hue(image, 0.8)
    return image, label


# args.tta_number = 0 means NO tta i.e. tta_func_list = [aug_0]
tta_func_list = [aug_0, aug_1, aug_2, aug_3, aug_4, aug_5, aug_6, aug_7, aug_8, aug_9,
                 aug_10, aug_11, aug_12, aug_13, aug_14, aug_15][:args.tta_number + 1]
...
def aug(image, label):
    ...
    #
        ...
        image, label = aug_10(image, label)
    elif aug_id == 10:
        image, label = aug_11(image, label)
    elif aug_id == 11:
        image, label = aug_12(image, label)
    elif aug_id == 12:
        image, label = aug_13(image, label)
    elif aug_id == 13:
        image, label = aug_14(image, label)
    elif aug_id == 14:
        image, label = aug_15(image, label)
    #
    return image, label
