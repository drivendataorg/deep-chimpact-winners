...
parser.add_str('--data_tfrec_dir',    default='../../data/tfrec_from_images_downsampled_step_2', help='Data directory containig TFRecord files')
...
parser.add_int('--aug_number',        default=12,            help='Number of train-time augmentations. 0 means no augmentation')
...
def aug_10(image, label): # gray
    ...
    return image, label
def aug_11(image, label): # blur_10
    image = tfa.image.gaussian_filter2d(image, filter_shape=10, sigma=5.0)
    return image, label
def aug_12(image, label): # blur_20
    image = tfa.image.gaussian_filter2d(image, filter_shape=20, sigma=5.0)
    return image, label
...
tta_func_list = [aug_0, aug_1, aug_2, aug_3, aug_4, aug_5, aug_6, aug_7, aug_8, aug_9,
                 aug_10, aug_11, aug_12][:args.tta_number + 1]
